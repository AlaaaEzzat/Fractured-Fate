...
public class DistanceShader : MonoBehaviour
{
    ...
    public void ReveilOjects()
    {
        ...
        if (activate)
        {
            distanceValue = Mathf.SmoothDamp(distanceValue, maxDistance, ref referancevelo , 15f);
        }
        else
        {
           distanceValue = Mathf.SmoothDamp(distanceValue, minmummDistance, ref referancevelo, 1f);
        }
    }
}
